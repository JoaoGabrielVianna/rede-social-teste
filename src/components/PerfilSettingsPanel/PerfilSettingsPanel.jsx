...
export default function PerfilSettingsPanel({ show = true, user_uid, user_password }) {
    const { logOut, deleteUserAccount } = useUserAuth()
    const options = [
        { id: 1, icon: help_icon, text: 'Configuracoes', onClick: () => alert('') },
        { id: 2, icon: help_icon, text: 'Deletar conta', onClick: () => deleteUserAccount({ uid: user_uid, password: user_password }) },
